  ...
  for (;;) {
    ...
    if (CANSIM == 0) {
      ...
      for (int x = 0; x < StringSize; x++) {
        ...
        for (int i = 0; i < (10*ModuleSize)/30; i++) {

          result = node.readHoldingRegisters(i * 30, 30);
          if (result == node.ku8MBSuccess)
          {
            for (int y = 0; y < 30; y++) {
              MasterArray[i * 30 + y + (10*ModuleSize * x)] = node.getResponseBuffer(y);
              Serial.println("Value of register#" + String(i * 30 + y + (10*ModuleSize * x)) + ": " + String(MasterArray[i * 30 + y + (10*ModuleSize * x)]));
            }
          }
          delay(50);
        }
    ...
    SystemSOC = getArrayAverage(StringSOCArray, StringSize);
